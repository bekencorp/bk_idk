...
boot_read_xip_status(uint8_t index, uint32_t type)
{
    ...
    {
    case XIP_MAGIC_TYPE:
        off = boot_xip_magic_off(index);
        break;
    case XIP_COPY_DONE_TYPE:
        off = boot_xip_copy_done_off(index);
        break;
    case XIP_IMAGE_OK_TYPE:
        off = boot_xip_image_ok_off(index);
        break;
    default:
        break;
    }
...
{
    if (image_index == 0) {
        return CONFIG_OW_OTA_CONTROL_PHY_PARTITION_OFFSET;
    } else {
        //return CONFIG_XIP_OTA_CONTROL_PHY_PARTITION_OFFSET;
        return 0;
    }
}
...
bk_boot_check_ota_confirm(uint8_t index, uint32_t value)
{
    ...
    BOOT_LOG_INF("get %d ota confirm=%#x",index, status);
    if (status == value) {
        return true;
    } else {
        return false;
...
{
    uint32_t check_value = 0;
    uint32_t offset = bk_boot_ota_confirm_offset(1);
    bk_flash_read_bytes(offset, (uint8_t*)&check_value, 4);
    if (check_value == UINT32_MAX) {
        bk_boot_write_ota_confirm(1, XIP_SET);
    } else {
        bk_boot_erase_ota_confirm(1);
    }
}
#endif
