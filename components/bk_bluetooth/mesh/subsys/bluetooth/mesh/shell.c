...
static int cmd_app_key_add(const struct shell *shell, size_t argc, char *argv[])
{
	...
	err = bt_mesh_app_key_add(key_app_idx, key_net_idx, key_val);

    if (err)
    {
        shell_error(shell, "App key add failed (err: %d)\n", err);
        return 0;
    }
...
int32_t zephry_mesh_shell_run(size_t argc, char **argv)
{
	...
	const struct shell_static_entry *entry = mesh_cmds.u.entry;
	uint32_t i = 0;
	...
	if(!strcasecmp("help", argv[0]))
	{
		uint32_t max_len = 0;

		while(entry[i].syntax)
		{
			if(strlen(entry[i].syntax) > max_len)
			{
				max_len = strlen(entry[i].syntax);
			}
		...
		while(entry[i].syntax)
		{
//			shell_print(NULL, "%-*s %s", max_len, entry[i].syntax, entry[i].help);
			BT_INFO("%d-%s %s\n", max_len, entry[i].syntax, entry[i].help);
			i++;
		}
	...
	while(entry[i].syntax)
	{
		if(!strcasecmp(argv[0], entry[i].syntax))
		{
			return entry[i].handler(NULL, argc, argv);
		}
...
uint8_t ble_mesh_shell_get_addr(uint8_t *buff)
{
