...
#if 0//CONFIG_ACLSEMI_BTC_FEATURE_INCLUDED //for ext
	           | BT_LE_ADV_OPT_EXT_ADV | BT_LE_ADV_OPT_USE_NAME | BT_LE_ADV_OPT_FORCE_NAME_IN_AD
#endif
			   ,
#endif
...
static int buf_send(struct net_buf *buf)
{
	...
	BT_VER("type %u len %u: %s", BT_MESH_ADV(buf)->type,
	       buf->len, bt_hex(buf->data, buf->len));
	BT_VER("count %u interval %ums duration %ums",
