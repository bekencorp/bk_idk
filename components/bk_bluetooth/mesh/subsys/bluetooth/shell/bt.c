		      ...
		      struct net_buf_simple *buf)
{
	...
	shell_print(ctx_shell, "C:%u S:%u D:%d SR:%u E:%u Prim: %s, Secn: %s, ",
	...
	shell_print(ctx_shell, "Interval: 0x%04x (%u ms), SID: 0x%x",
...
int32_t zephry_bt_shell_run(size_t argc, char **argv)
{
	...
	const struct shell_static_entry *entry = bt_cmds.u.entry;
	uint32_t i = 0;
	...
	while(entry[i].syntax)
	{
		...
		{
		    if (argc < entry[i].args.mandatory)
            {
                BT_INFO("param is not enough\n");
                return 0;
            }
