...
extern int32_t mesh_foreach_bt_conn_cb(int32_t (*cb)(struct bt_conn_cb *param, void *arg1, void *arg2, void *arg3, void *arg4), void *arg1, void *arg2, void *arg3, void *arg4);

int32_t ethermind_bt_conn_connected_callback(struct bt_conn_cb *param, void *arg1, void *arg2, void *arg3, void *arg4)
{
	struct bt_conn *conn = (typeof(conn))arg1;
	if (param->connected)
	{
		param->connected(conn, conn->err);
	}
	return true;
}

int32_t ethermind_bt_conn_disconnected_callback(struct bt_conn_cb *param, void *arg1, void *arg2, void *arg3, void *arg4)
{
	struct bt_conn *conn = (typeof(conn))arg1;
	if (param->disconnected)
	{
		param->disconnected(conn, conn->err);
	}
	return true;
}

#if defined(CONFIG_BT_REMOTE_INFO)
int32_t ethermind_bt_conn_remote_info_available_callback(struct bt_conn_cb *param, void *arg1, void *arg2, void *arg3, void *arg4)
{
	struct bt_conn *conn = (typeof(conn))arg1;
	struct bt_conn_remote_info *remote_info = (typeof(remote_info))arg2;
	if (param->remote_info_available)
	{
		param->remote_info_available(conn, remote_info);
	}
	return true;
}
#endif

int32_t ethermind_bt_conn_le_param_updated_callback(struct bt_conn_cb *param, void *arg1, void *arg2, void *arg3, void *arg4)
{
	struct bt_conn *conn = (typeof(conn))arg1;
	if (param->le_param_updated)
	{
		param->le_param_updated(conn, conn->le.interval,
						conn->le.latency,
						conn->le.timeout);
	}
	return true;
...
int32_t ethermind_bt_conn_le_data_len_updated_callback(struct bt_conn_cb *param, void *arg1, void *arg2, void *arg3, void *arg4)
{
	struct bt_conn *conn = (typeof(conn))arg1;
	if (param->le_data_len_updated)
	{
		param->le_data_len_updated(conn, conn->le.data_len);
	}
	return true;
}
#endif
...
int32_t ethermind_bt_conn_le_phy_updated_callback(struct bt_conn_cb *param, void *arg1, void *arg2, void *arg3, void *arg4)
{
	struct bt_conn *conn = (typeof(conn))arg1;
	if (param->le_phy_updated)
	{
		param->le_phy_updated(conn, conn->le.phy);
	}
	return true;
}
#endif

int32_t ethermind_bt_conn_le_param_req_callback(struct bt_conn_cb *param, void *arg1, void *arg2, void *arg3, void *arg4)
{
	struct bt_conn *conn = (typeof(conn))arg1;
    struct bt_le_conn_param *conn_param = (typeof(conn_param))arg2;
	if (param->le_param_req)
	{
		if(!param->le_param_req(conn, conn_param))
		{
			return false;
		}
...
int32_t ethermind_bt_conn_identity_resolved_callback(struct bt_conn_cb *param, void *arg1, void *arg2, void *arg3, void *arg4)
{
	struct bt_conn *conn = (typeof(conn))arg1;
    bt_addr_le_t *rpa = (typeof(rpa))arg2;
	if (param->identity_resolved)
	{
		param->identity_resolved(conn, rpa, &conn->le.dst);
	}
	return true;
}

int32_t ethermind_bt_conn_security_changed_callback(struct bt_conn_cb *param, void *arg1, void *arg2, void *arg3, void *arg4)
{
	struct bt_conn *conn = (typeof(conn))arg1;
    enum bt_security_err *err = (typeof(err))arg2;
	if (param->security_changed)
	{
		param->security_changed(conn, conn->sec_level, *err);
	}
	return true;
}
...
#if CONFIG_ACLSEMI_BTC_FEATURE_INCLUDED
	mesh_foreach_bt_conn_cb(ethermind_bt_conn_connected_callback, conn, NULL, NULL, NULL);
#else
...
#if CONFIG_ACLSEMI_BTC_FEATURE_INCLUDED
	mesh_foreach_bt_conn_cb(ethermind_bt_conn_disconnected_callback, conn, NULL, NULL, NULL);
#else
	STRUCT_SECTION_FOREACH(bt_conn_cb, cb) {
		if (cb->disconnected) {
			cb->disconnected(conn, conn->err);
...
#if CONFIG_ACLSEMI_BTC_FEATURE_INCLUDED
	mesh_foreach_bt_conn_cb(ethermind_bt_conn_remote_info_available_callback, conn, &remote_info, NULL, NULL);
#else
	STRUCT_SECTION_FOREACH(bt_conn_cb, cb) {
		if (cb->remote_info_available) {
			cb->remote_info_available(conn, &remote_info);
...
#if CONFIG_ACLSEMI_BTC_FEATURE_INCLUDED
	mesh_foreach_bt_conn_cb(ethermind_bt_conn_le_param_updated_callback, conn, NULL, NULL, NULL);
#else
	STRUCT_SECTION_FOREACH(bt_conn_cb, cb) {
		if (cb->le_param_updated) {
			cb->le_param_updated(conn, conn->le.interval,
...
#if CONFIG_ACLSEMI_BTC_FEATURE_INCLUDED
	mesh_foreach_bt_conn_cb(ethermind_bt_conn_le_data_len_updated_callback, conn, NULL, NULL, NULL);
#else
	STRUCT_SECTION_FOREACH(bt_conn_cb, cb) {
		if (cb->le_data_len_updated) {
			cb->le_data_len_updated(conn, &conn->le.data_len);
...
#if CONFIG_ACLSEMI_BTC_FEATURE_INCLUDED
	mesh_foreach_bt_conn_cb(ethermind_bt_conn_le_phy_updated_callback, conn, NULL, NULL, NULL);
#else
	STRUCT_SECTION_FOREACH(bt_conn_cb, cb) {
		if (cb->le_phy_updated) {
			cb->le_phy_updated(conn, &conn->le.phy);
...
#if CONFIG_ACLSEMI_BTC_FEATURE_INCLUDED
	return mesh_foreach_bt_conn_cb(ethermind_bt_conn_le_param_req_callback, conn, param, NULL, NULL);
#else
	STRUCT_SECTION_FOREACH(bt_conn_cb, cb) {
		if (!cb->le_param_req) {
			continue;
...
#if CONFIG_ACLSEMI_BTC_FEATURE_INCLUDED
	mesh_foreach_bt_conn_cb(ethermind_bt_conn_identity_resolved_callback, conn, (bt_addr_le_t *)rpa, NULL, NULL);
#else
	STRUCT_SECTION_FOREACH(bt_conn_cb, cb) {
		if (cb->identity_resolved) {
			cb->identity_resolved(conn, rpa, &conn->le.dst);
...
#if CONFIG_ACLSEMI_BTC_FEATURE_INCLUDED
	mesh_foreach_bt_conn_cb(ethermind_bt_conn_security_changed_callback, conn, &err, NULL, NULL);
#else
	STRUCT_SECTION_FOREACH(bt_conn_cb, cb) {
		if (cb->security_changed) {
			cb->security_changed(conn, conn->sec_level, err);
