...
static struct bt_le_ext_adv *adv_new(void)
{
	...
	i = CONFIG_BT_EXT_ADV_MAX_ADV_SET - 1;
#endif

	for (; i < ARRAY_SIZE(adv_pool); i++) {
		...
		for (size_t j = 0; j < ad[i].len; j++) {
			...
			if ((set_data_len + len + 2) > set_data_len_max) {
				...
				if (!(type == BT_DATA_NAME_COMPLETE &&
				      shortened_len > 0)) {
					BT_ERR("Too big advertising data 0x%02X", type);
					return -EINVAL;
				}
...
#if CONFIG_ACLSEMI_BTC_FEATURE_INCLUDED
	{
		struct bt_hci_cmd_hdr *hdr = (typeof(hdr))(buf->data);
		hdr->param_len = set_data->len + 4;
		net_buf_remove_mem(buf, sizeof(set_data->data) - set_data->len);
	}
#endif
...
void bt_hci_le_adv_set_terminated(struct net_buf *buf)
{
	...
	if(!adv)
	{
	    BT_WARN("unknow adv handle %d, maybe ble host", evt->adv_handle);
        return;
	}
#endif
