...
struct net_buf_pool *_net_buf_pool_list[NET_BUF_POOL_LIST_SIZE] = {0};

struct net_buf_pool *net_buf_pool_get(int id)
{
	///return &_net_buf_pool_list[id];
	if(id >= 0 && id < NET_BUF_POOL_LIST_SIZE){
		return _net_buf_pool_list[id];
	}else{
		return NULL;
	}
...
static int pool_id(struct net_buf_pool *pool)
{
	...
	int i;
	for(i = 0;i<NET_BUF_POOL_LIST_SIZE;i++){
		if(_net_buf_pool_list[i] == pool){
			return i;
		}
	}
	unsigned int key = irq_lock();
	for(i = 0;i<NET_BUF_POOL_LIST_SIZE;i++){
		if(_net_buf_pool_list[i] == NULL){
			_net_buf_pool_list[i] = pool;
			irq_unlock(key);
			return i;
		}
	}
...
static uint8_t net_buf_get_free_total(struct k_queue *queue)
{
    sys_snode_t * node = queue->data_q.head;
    uint8_t num = 0;
    while (node) {
        if (node == NULL) {
            break;
        }
        node = z_snode_next_peek(node);
        num++;
    }
    return num;
}
...
success:
	...
	if (size) {
		...
		if (!K_TIMEOUT_EQ(timeout, K_NO_WAIT) &&
			...
			int32_t remaining = end - z_tick_get();
...
/////////////////////////// Test demo ///////////////////////////////////////
#if defined(Z_NET_BUF_TEST_CFG) && Z_NET_BUF_TEST_CFG
#include "string.h"
#include "stdio.h"
#include "stdlib.h"
//#include <os/mem.h>
...
#define NETBUF_TEST_SIZE         32
#define NETBUF_TEST_BUF_COUNT    10

NET_BUF_POOL_FIXED_DEFINE(net_buf_test_pool, NETBUF_TEST_BUF_COUNT,
			  NETBUF_TEST_SIZE, NULL);

#define NETBUF_FIFO_NB   2
#define TNB_BUF_RESERVE  2
#define NETBUF_EVT_NB    2
static struct k_fifo _netbuf_fifo[NETBUF_FIFO_NB];
...
enum test_netbuf_type {
	TNB_BUF_CMD,
	TNB_BUF_EVT,
};

static void netbuf_process_events(struct k_poll_event *ev, int count,struct k_fifo *fifo)
{
	int m_count = count;
		...
		case K_POLL_STATE_FIFO_DATA_AVAILABLE:
			{
				...
				buf = net_buf_get(&fifo[m_count - count], K_NO_WAIT);
				///BT_ASSERT(buf);
				BT_INFO("get buf:%p\r\n",buf);
				if ( buf ) {
					BT_INFO("net_buf_unref:%p\r\n",buf);
					net_buf_unref(buf);
					BT_INFO("net_buf_unref:%p end\r\n",buf);
				}
			}
			break;
...
static void netbuf_thread_function( void* arg)
{
	struct k_fifo *__fifo = (struct k_fifo *)arg;
	int idx = (__fifo == &_netbuf_fifo[0]) ? 0 : 1;
	static struct k_poll_event events[NETBUF_EVT_NB] = {
	...
	BT_INFO("%s(thd:%p) idx:%d run\r\n",__FUNCTION__,thd,idx);
	if(k_fifo_is_uninitialized(&_netbuf_fifo[0]) != 0) {
		k_fifo_init(&_netbuf_fifo[0]);
	}
	if(k_fifo_is_uninitialized(&_netbuf_fifo[1]) != 0){
		k_fifo_init(&_netbuf_fifo[1]);
	}

	while(1) {
		for(ev_count = 0; ev_count < NETBUF_EVT_NB; ev_count++){
			events[ev_count].state = K_POLL_STATE_NOT_READY;
		}

		err = k_poll(events, ev_count, K_FOREVER);
		if(err == 0) {
			netbuf_process_events(events, ev_count,&_netbuf_fifo[0]);
		}
		else {
			BT_INFO("k_poll(%p) return %d error\r\n",thd,err);
		}
...
void z_net_buf_test_command(int argc, char **argv)
{
	int idx = 0;
	if ( strcmp(argv[1], "buf_init") == 0 ) {
		void* qthread = {0};
		if ( argc == 3 ) {
			idx = atoi(argv[2]);
			if(idx >= NETBUF_FIFO_NB){
				BT_INFO("nebuf index(%d) > %d error\r\n",idx,NETBUF_FIFO_NB);
				return;
			}
		}

		if((k_fifo_is_uninitialized(&_netbuf_fifo[0]) == 0)
			|| (k_fifo_is_uninitialized(&_netbuf_fifo[1]) == 0)){
			BT_INFO("_netbuf_fifo %d was initialized\r\n",idx);
			return;
		}
		k_fifo_init(&_netbuf_fifo[idx]);
		rtos_create_thread(&qthread,
							7,
							"zqt",
							netbuf_thread_function,
							4*1024,
							&_netbuf_fifo[idx]);

	}
	else if ( strcmp(argv[1], "buf_append") == 0 ) {
		struct net_buf *buf;
		k_timeout_t timeout;
		if(k_fifo_is_uninitialized(&_netbuf_fifo[idx]) != 0){
			k_fifo_init(&_netbuf_fifo[idx]);
		}
		timeout.ticks = 500;
		buf = net_buf_alloc(&net_buf_test_pool, timeout);
		if (!buf) {
			BT_INFO("net_buf_alloc %d failed\r\n",idx);
			return;
		}
		net_buf_reserve(buf, TNB_BUF_RESERVE);
		///bt_buf_set_type(buf, TNB_BUF_CMD);

		BT_INFO("net_buf_put,buf:%p\r\n",buf);
		net_buf_put(&_netbuf_fifo[idx], buf);
		BT_INFO("net_buf_put,buf:%p exit\r\n",buf);
	}else if ( strcmp(argv[1], "buf_append2") == 0 ) {
		struct net_buf *buf;
		k_timeout_t timeout;
		idx = 1;
		if(k_fifo_is_uninitialized(&_netbuf_fifo[idx]) != 0){
		...
		BT_INFO("net_buf_put,2 buf:%p\r\n",buf);
		net_buf_put(&_netbuf_fifo[idx], buf);
		BT_INFO("net_buf_put,2 buf:%p exit\r\n",buf);
	}
}
