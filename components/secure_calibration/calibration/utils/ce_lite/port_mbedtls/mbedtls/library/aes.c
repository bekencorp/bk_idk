...
#if 1//defined(MBEDTLS_CIPHER_MODE_CBC)
/*
...
#define AES_SELF_PORT_TETS_OK 1
#if AES_SELF_PORT_TETS_OK

static const unsigned char aes_test_cbc_plain_text[256] = {
...
#else
static const unsigned char aes_test_cbc_key_val[32] = {0x0,0x1,0x2,0x3,0x4,0x5,0x6,0x7,0x8,0x9,0xA,0xB,0xC,0xD,0xE,0xF,0x0,0x1,0x2,0x3,0x4,0x5,0x6,0x7,0x8,0x9,0xA,0xB,0xC,0xD,0xE,0xF};
static const unsigned char aes_test_cbc_iv_val[16] = {0x0,0x1,0x2,0x3,0x4,0x5,0x6,0x7,0x8,0x9,0xA,0xB,0xC,0xD,0xE,0xF};
#endif



static const unsigned char aes_test_cbc_cipher_text[256] = {
0xbc, 0xf2, 0x3f, 0x2, 0xa5, 0x4d, 0x5a, 0xce, 0xa, 0x97, 0x6f, 0x5c, 0x89, 0x17, 0x8e, 0x3b,
0x7, 0xdc, 0xb8, 0x77, 0x95, 0x7, 0x3f, 0x48, 0x59, 0x10, 0x5a, 0x2b, 0xdb, 0xa5, 0xc5, 0xe, 
...
0x8, 0x6, 0x85, 0x80, 0x42, 0xe2, 0x43, 0x29, 0x16, 0xac, 0x7, 0x8a, 0x48, 0xc7, 0xb6, 0x45, 0x65, 
0x84, 0x79, 0x50, 0x13, 0xee, 0x3e, 0x6c, 0x29, 0xa5, 0xa5, 0x61, 0x15, 0xbd, 0x6c, 0xde, 0xbe, 
0x4e, 0x18, 0x87, 0xb8, 0xaa, 0x94, 0xdd, 0x76, 0xa9, 0x29, 0xdc, 0x47, 0xd0, 0xb1, 0xe2, 0xc7, 
0xd2, 0x22, 0xe7, 0x80, 0xee, 0xe9, 0xb4, 0xd3, 0x2, 0xcb, 0x54, 0x93, 0x17, 0x7e, 0x19, 0x33,
0x17, 0xa2, 0x44, 0x91, 0x27, 0xf6, 0x77, 0xe6, 0x2c, 0x9a, 0xb0, 0x49, 0xe2, 0x1, 0xfd, 0x59, 
0x0, 0x31, 0xd3, 0xf6, 0xfe, 0x91, 0xd6, 0xd6, 0x2f, 0xe3, 0x8e, 0xc6, 0xcb, 0x21, 0x3d, 0xb8, 
0x4e, 0xe, 0x8c, 0x6f, 0x19, 0x7b, 0xc, 0xdf, 0xd2, 0xd9, 0x17, 0x54, 0x10, 0xe4, 0x11, 0x86, 
0xcb, 0x2d, 0xff, 0x72, 0x1e, 0xae, 0x54, 0xf4, 0x28, 0xc, 0xd6, 0xa8, 0x56, 0x78, 0x2d, 0xb2, 
0x5, 0xd3, 0x23, 0x83, 0x33, 0x68, 0x58, 0x4e, 0xbc, 0xbd, 0x7f, 0xd2, 0x9b, 0xa1, 0x76, 0x39, 
0x9e, 0x9f, 0xaa, 0x6, 0x83, 0x41, 0xa, 0xf7, 0x93, 0xdb, 0x79, 0xbd, 0xba, 0xf4, 0xc3, 0x6a, 
0x50, 0xfa, 0xc2, 0x39, 0xb6, 0x34, 0xeb, 0x40, 0xb0, 0x87, 0x57, 0x6a, 0xc1, 0xed, 0xbe};

#endif /* CE_LITE_AES && CE_LITE_CIPHER_MODE_CBC */

//[CYG-key256_CBC-NOPADDING test ok]



int mbedtls_aes_port_test(int verbose,int mode)
{
	...
	keybits = (sizeof(aes_test_cbc_key_val) << 3);

    if (mode == MBEDTLS_AES_DECRYPT) {
        ret = mbedtls_aes_setkey_dec(&ctx, aes_test_cbc_key_val, keybits);
        aes_tests = aes_test_cbc_cipher_text;
    } else {
        ret = mbedtls_aes_setkey_enc(&ctx, aes_test_cbc_key_val, keybits);
        aes_tests = aes_test_cbc_plain_text;
    }
    
    memcpy(iv, aes_test_cbc_iv_val, sizeof(aes_test_cbc_iv_val));

    ret = mbedtls_aes_crypt_cbc(&ctx, mode, sizeof(buf), iv, aes_tests, buf);
	...
	if (memcmp(buf, aes_tests, sizeof(buf)) != 0) 
	{
            ret = 1;
            goto exit;
    }
...
#define AES_KEY_LEGNTH        (32)
#define AES_IV_LENGTH         (16)
#define AES_KEY_SHITF_BIT_NUM (3)

int aes_decrypt_with_aeskey(unsigned char *key_val, unsigned char *iv_val, unsigned char *encrypt_text , unsigned char *plain_text)
{
    int ret = 0;
    unsigned int keybits;
    unsigned char aes_cbc_key_val[AES_KEY_LEGNTH];
    unsigned char aes_cbc_iv_val[AES_IV_LENGTH];
    mbedtls_aes_context ctx;

    mbedtls_printf(" enter.\n",__FUNCTION__);
    mbedtls_aes_init(&ctx);

    memcpy(aes_cbc_key_val,key_val,AES_KEY_LEGNTH);

    memcpy(aes_cbc_iv_val,iv_val,AES_IV_LENGTH);

    keybits = (sizeof(aes_cbc_key_val) << AES_KEY_SHITF_BIT_NUM) ;

    mbedtls_printf("keybits :0x%x.\n", keybits);

    ret = mbedtls_aes_setkey_dec(&ctx, aes_cbc_key_val, keybits);

    ret = mbedtls_aes_crypt_cbc(&ctx, MBEDTLS_AES_DECRYPT, keybits, aes_cbc_iv_val, encrypt_text, plain_text);
