...
void test_fatfs_perf_with_param(DISK_NUMBER number, uint32_t TEST_BUF_SIZE, uint32_t TEST_ITERATIONS) {
    ...
    char cFileName[FF_MAX_LFN];
    FATFS_LOGD("\r\n----- %s %d start -----\r\n", __func__, number);
    sprintf(cFileName, "%d:%s", number, TEST_PERF_FILE_NAME);
    FATFS_LOGD("f_open \"%s\"\r\n", cFileName);

    if(buffer == NULL){
        FATFS_LOGE("%s os_malloc fail, size = %d. \n", TEST_BUF_SIZE);
        return;
    }
    ...
    for(int i=0; i<TEST_ITERATIONS; i++) {
        res = f_open(&file, cFileName, FA_WRITE | FA_CREATE_ALWAYS);
        if(res != FR_OK) {
            FATFS_LOGE("Open for write failed: %d\n", res);
            break;
        }

        res = f_write(&file, buffer, TEST_BUF_SIZE, &bw);
        if(res != FR_OK || bw != TEST_BUF_SIZE) {
            FATFS_LOGE("Write failed: %d, bytes: %d\n", res, bw);
        }

        f_close(&file);
    }
    end_time = rtos_get_time();
    FATFS_LOGI(">>> Write test: %d iterations, %d bytes each, total time: %d ms\n", 
    ...
    if(f_perf_speed < 0) {
        FATFS_LOGE("[ERROR] Write test failed\n");
    } else {
        FATFS_LOGI("===== Write Throughput: %.2f KB/s======\n", f_perf_speed);
    }
    ...
    start_time = rtos_get_time();
    for(int i=0; i<TEST_ITERATIONS; i++) {
        res = f_open(&file, cFileName, FA_READ);
        if(res != FR_OK) {
            FATFS_LOGE("Open for read failed: %d\n", res);
            break;
        }
        
        res = f_read(&file, buffer, TEST_BUF_SIZE, &bw);
        if(res != FR_OK || bw != TEST_BUF_SIZE) {
            FATFS_LOGE("Read failed: %d, bytes: %d\n", res, bw);
        }
        
        f_close(&file);
    }
    end_time = rtos_get_time();
    FATFS_LOGI(">>> Read test: %d iterations, %d bytes each, total time: %d ms\n", 
             TEST_ITERATIONS, TEST_BUF_SIZE, end_time - start_time);

    total_time = end_time - start_time;
    f_perf_speed = (TEST_BUF_SIZE * TEST_ITERATIONS) / (total_time / 1000.0f) / 1024;
    if(f_perf_speed < 0) {
        FATFS_LOGE("[ERROR] Read test failed\n");
    } else {
        FATFS_LOGI("===== Read Throughput: %.2f KB/s======\n", f_perf_speed);
    }
    os_free(buffer);
}

void test_fatfs_performance(DISK_NUMBER number) {
	BK_DUMP_OUT("+++++++++++++++++++++++++\r\n");
	test_fatfs_perf_with_param(number, 102400, 100);
	rtos_delay_milliseconds(50);
	BK_DUMP_OUT("-------------------------\r\n\r\n\r\n\r\n");

	BK_DUMP_OUT("+++++++++++++++++++++++++\r\n");
	test_fatfs_perf_with_param(number, 10240, 100);
	rtos_delay_milliseconds(50);
	BK_DUMP_OUT("-------------------------\r\n\r\n\r\n\r\n");

	BK_DUMP_OUT("+++++++++++++++++++++++++\r\n");
	test_fatfs_perf_with_param(number, 8192, 100);
	rtos_delay_milliseconds(50);
	BK_DUMP_OUT("-------------------------\r\n\r\n\r\n\r\n");

	BK_DUMP_OUT("+++++++++++++++++++++++++\r\n");
	test_fatfs_perf_with_param(number, 2048, 100);
	rtos_delay_milliseconds(50);
	BK_DUMP_OUT("-------------------------\r\n\r\n\r\n\r\n");

	BK_DUMP_OUT("+++++++++++++++++++++++++\r\n");
	test_fatfs_perf_with_param(number, 1024, 100);
	rtos_delay_milliseconds(50);
	BK_DUMP_OUT("-------------------------\r\n\r\n\r\n\r\n");

	BK_DUMP_OUT("+++++++++++++++++++++++++\r\n");
	test_fatfs_perf_with_param(number, 640, 100);
	rtos_delay_milliseconds(50);
	BK_DUMP_OUT("-------------------------\r\n\r\n\r\n\r\n");
