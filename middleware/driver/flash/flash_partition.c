...
#if CONFIG_PARTITION_OP_BY_NAME
#if (CONFIG_TFM_FWU)
...
const char *s_partition_name[PARTITION_CNT] = {
	...
	const bk_logic_partition_t logic_partitions[] = PARTITION_MAP;
...
const bk_logic_partition_t * get_partition_info_by_name(const char *label) {
	for (size_t i = 0; i < sizeof(logic_partitions) / sizeof(logic_partitions[0]); ++i) {
		if (strcmp(logic_partitions[i].partition_description, label) == 0) {
...
#elif (CONFIG_SOC_BK7236XX) || (CONFIG_SOC_BK7239XX)
#if CONFIG_PARTITION_OP_BY_NAME
		pt = (bk_logic_partition_t*) get_partition_info(partition);
#else
		pt = (bk_logic_partition_t *)&bk_flash_partitions[partition];
#endif
#else
