...
typedef struct
{
	void *(*frame_init) (uint16_t sensor_id, uint8_t camera_type, uint16_t image_format); /**< init frame buffer */
	int (*frame_deinit) (void *node); /**< deinit frame buffer */
	int (*frame_clear) (void *node); /**< clear frame buffer */
	int (*frame_complete) (uint16_t image_format, void *node, frame_buffer_t* buffer);  /**< dvp notify upper layer a complete frame ready */
	frame_buffer_t* (*frame_malloc)(uint16_t image_format, void *node, uint32_t size); /**< malloc jpeg psram buffer */
