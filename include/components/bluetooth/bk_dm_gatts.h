...
typedef enum
{
    ...
    BK_GATTS_STOP_EVT                = 13,      /*!< When stop service complete, the event comes */
...
#define BK_GATT_PREP_WRITE_EXEC   0x01 /*!< Prepare write flag to indicate execute prepare write */
    ...
    struct gatts_conf_evt_param
    {
        bk_gatt_status_t status;       /*!< Operation status */
        uint16_t conn_id;               /*!< Connection id */
        uint16_t handle;                /*!< attribute handle. Not used when send indicate */
    ...
    /**
     * @brief BK_GATTS_STOP_EVT
     */
    struct gatts_stop_evt_param {
        bk_gatt_status_t status;       /*!< Operation status */
        uint16_t service_handle;        /*!< Service attribute handle */
    } stop;                             /*!< Gatt server callback param of BK_GATTS_STOP_EVT */
    ...
    /**
     * @brief BK_GATTS_SET_ATTR_VAL_EVT
     */
     struct gatts_set_attr_val_evt_param
     {
         ...
         bk_gatt_status_t status;       /*!< Operation status*/
     } set_attr_val;                     /*!< Gatt server callback param of BK_GATTS_SET_ATTR_VAL_EVT */

     /**
     * @brief BK_GATTS_SEND_SERVICE_CHANGE_EVT
     */
     struct gatts_send_service_change_evt_param
     {
         ...
         uint16_t conn_id;               /*!< Connection id */
         bk_bd_addr_t remote_bda;       /*!< Remote bluetooth device address */
     } service_change;                    /*!< Gatt server callback param of BK_GATTS_SEND_SERVICE_CHANGE_EVT */
...
/**
 * @brief               Create a service attribute tab. Only one service once.
 * @param[in]       gatts_attr_db: the pointer to the service attr tab. At least has one service declar
 * @param[in]       gatts_if: GATT server access interface
 * @param[in]       count: the number of gatts_attr_db.
 * @param[in]       max_attr_count: max attr num of this service.
 *
 * @return
 *                  - BK_ERR_BLE_SUCCESS : success
 *                  - other  : failed
...
ble_err_t bk_ble_gatts_create_attr_tab(const bk_gatts_attr_db_t *gatts_attr_db,
                                        ...
                                        uint16_t count, uint32_t max_attr_count);
 ...
 *
 * @param[in]       service_attr_handle: the service handle to be started.
 *
 * @return
 *                  - BK_ERR_BLE_SUCCESS : success
 *                  - other  : failed
 ...
 *
 * @param[in]       service_attr_handle: the service handle to be stop.
 *
 * @return
 *                  - BK_ERR_BLE_SUCCESS : success
 *                  - other  : failed
...
/**
 * @brief           Send service change indicate
 *
 * @param[in]       gatts_if: GATT server access interface
 * @param[in]       conn_id - connection handle.
 * @param[in]       all_connected - If all_connected is true then it will send service change
 *                  indicate to all the connected devices and conn_id will be ignore.
 *
 * @return
 *                  - BK_ERR_BLE_SUCCESS : success
 *                  - other  : failed
