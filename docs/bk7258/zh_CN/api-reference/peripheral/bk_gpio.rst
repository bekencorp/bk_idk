...
------------------
  ...
  GPIO is configured according to the MAP(GPIO_DEFAULT_DEV_CONFIG) during driver initialization of each CPU core. Each row in the map contains nine elements.
...
------------------
  ...
  3. Currently, the BK7258 adopts a basic AMP architecture, which may cause GPIO interrupts to be handled bu multiple CPU cores, affecting the program's expected behavior(Because CPU1 has
     a higher clock speed than CPU0, this may lead to interrupt loss). To resovle this issue, the following measures are proposed:

    - By following the second approach, a custom GPIO map table can be defined for each CPU core, with different GPIOs assigned to different cores. This way, each CPU core will only respond
  ...
  4. Low power state: Can be configured as input mode and output mode, and do not need to care about the GPIO during low power mode. If the corresponding GPIO (GPIO_LOW_POWER_DISCARD_IO_STATUS) 
  ...
  5. There are **two ways to configure** low power states:
  ...
  6. When you need to **maintain the output status** of GPIO in low-power mode, you can configure the corresponding GPIO through the third method.
  7. When entering low-power mode, multiple external GPIOs can be set as wake-up sources. If any GPIO has an interrupt signal, the chip can wake up from low-power mode. For example, you can set 
     multiple wake-up sources in **GPIO_STATIC_WAKEUP_SOURCE_MAP**.
  8. When entering low power mode(Low voltage only), the status of GPIO will be backed up. After exiting, GPIO will be automatically restored to the state before entering low power mode.
