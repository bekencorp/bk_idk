...
本文档以 BK7286 开发板为例，通过一个简单的示例项目向您展示:
...
点击 `下载 <https://dl.bekencorp.com/tools/toolchain/arm/gcc-arm-none-eabi-10.3-2021.10-x86_64-linux.tar.bz2>`_ 下载 BK7286 工具链。
...
.. note::
    ...
    工具链默认路径在 middleware/soc/bk7286/bk7286.defconfig 文件中定义，您也可以在项目配置文件中更改工具链路径::
...
在终端下输入下述命令编译 Armino BK7286 verify 工程::

    cd ~/armino
    make bk7286 PROJECT=verify/spe

您也可以使用 verify 工程特有的编译命令来编译 BK7286 默认工程::
...
.. note::
  ...
  Armino 框架支持通过配置 PROJECT 参数来编译任意工程，但 BK7286 尚在开发阶段，当前仅支持 verify 工程编译。
...
您可以通过工程配置文件来进行更改默认配置或者针对不同芯片进行差异化配置::
    ...
    如： bk7286.config >> bk7286.defconfig >> KConfig
    + 工程配置文件示例：
        projects/verify/config/bk7286.config
    + 芯片配置文件示例：
...
BK7286 默认工程为 projects/app，新建工程可参考 projects/harmony 工程
...
.. figure:: ../../_static/download_uart_bk7286.png
    :align: center
    :alt: BKFIL GUI
    :figclass: align-center
...
目前 BK7286 平台，串口 Log 及 Cli 命令输入在 DL_UART0 口；可通过 help 命令查看支持命令列表。
