...
可以将这些文件归为两类：
 ...
 - 配置修改文件:
   ...
   - bk7286.defconfig - Armino 提供的 BK7286 默认配置。
   - bk7286.config - 工程中 BK7286 专有配置。
...
.. note::
  ...
  会在 menuconfig 顶层菜单中显示该配置项。BK7286 不支持 menuconfig，因此 Kconfig.projbuild
  与 Kconfig 作用相同。

在 Armino 工程中，Kconfig 配置优先级为:

  bk7286.config > bk7286.defconfig > 配置定义文件

menuconfig 支持
-------------------------

BK7286 Kconfig 配置不支持 menuconfig 配置。
