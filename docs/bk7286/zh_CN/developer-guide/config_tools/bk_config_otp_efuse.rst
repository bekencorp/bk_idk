...
脚本分四个逻辑配置区：
 ...
 - 安全控制区 - 由 ``Security_Ctrl_Enable`` 与 ``Security_Ctrl`` 配置，在 BK7286 中指 EFUSE 配置。
 - 安全数据区 - 由 ``Security_Data_Enable`` 与 ``Security_Data`` 配置，在 BK7286 中指 OTP 配置。
 - EFUSE 区 - 由 ``eFuse_Enable`` 与 ``eFuse`` 配置，在 BK7286 中该字段已废弃。
