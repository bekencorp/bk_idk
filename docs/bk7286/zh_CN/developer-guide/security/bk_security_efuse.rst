...
EFUSE (Electrically Programming Efuse) 用于配置 BK7286 中与安全相关的特性，只能由 0 配置成 1，无法由 1 配置成 0。

BK7286 EFUSE 共有 4x8 共 32 Bits, 在安全启动使能前，可以通过 :ref:`BKFIL<bk_tool_bkfil>` 配置，详情请参考 :ref:`EFUSE 配置 <bk_config_otp_efuse>`。
...
BIT(21) 用于禁用 SPI 到 AHB 通道：
 ...
 - 0 - SPI 到 AHB 通道使能。此时，通过 SPI 接口可直接操作 BK7286 寄存器。
 - 1 - SPI 到 AHB 通道关闭。此时，无法通过 SPI 接口操作 BK7286 寄存器。
