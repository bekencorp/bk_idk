...
6.1 FWU提供的接口
------------------------
TFM提供强制函数与非强制函数，bk7286只用到了强制函数。
...
6.2.1 打开TF-M的FWU功能
------------------------
在bk7286的配置文件中，打开TFM_PARTITION_FIRMWARE_UPDATE。由于FWU跟MCUBOOT共用一套flash接口，需要同时打开MCUBOOT_DATA_SHARING，并使能BL2。
...
7.1 增加TF-M的暂存区配置
------------------------
由于TF-M默认只有一个暂存区，无法满足bk7286升级BL2的需求。增加TF-M中4个暂存区为：
