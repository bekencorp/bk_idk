...
----------------------------------
TFM provides mandatory and unmandatory functions, while bk7286 uses only mandatory functions.
...
---------------------------------------
In the configuration file of the bk7286, open TFM_PARTITION_FIRMWARE_UPDATE. Since FWU and MCUBOOT share the same flash interface, it is necessary to open MCUBOOT_DATA_SHARING and enable BL2 at the same time.
...
------------------------------------------------
Since TF-M has only one staging area by default, it cannot meet the needs of bk7286 to upgrade BL2. Increase the four staging areas in TF-M as follows.
