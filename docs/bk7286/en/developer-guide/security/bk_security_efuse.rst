...
EFUSE (Electrically Programming Efuse) is used to configure safety-related features in BK7286. It can only be configured from 0 to 1, and cannot be configured from 1 to 0.

BK7286 EFUSE has a total of 4x8 and a total of 32 Bits. Before secure boot is enabled, it can be configured through :ref:`BKFIL<bk_tool_bkfil>`. For details, please refer to :ref:`EFUSE configuration <bk_config_otp_efuse>`.
...
BIT(21) is used to disable SPI to AHB channel:
  ...
  - 0 - SPI to AHB channel enable. At this time, the BK7286 registers can be directly operated through the SPI interface.
  - 1 - SPI to AHB channel off. At this time, the BK7286 registers cannot be operated through the SPI interface.
...
BIT(31) is used to control the switch of CPU debug port:
  ...
  - 0 - CPU debugging is enabled, BK7286 supports SWD debugging.
