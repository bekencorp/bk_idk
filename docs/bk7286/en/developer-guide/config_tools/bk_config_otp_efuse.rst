...
The script is divided into four logical configuration areas:
  ...
  - Security control area - configured by ``Security_Ctrl_Enable`` and ``Security_Ctrl``, which refers to EFUSE configuration in BK7286.
  - Security data area - configured by ``Security_Data_Enable`` and ``Security_Data``, which refers to OTP configuration in BK7286.
  - EFUSE field - configured by ``eFuse_Enable`` and ``eFuse``, this field is deprecated in BK7286.
...
Global fields specifically refer to the first-level fields in the configuration table:
  ...
  - ``eFuse_Enable`` - Obsolete in BK7286.
