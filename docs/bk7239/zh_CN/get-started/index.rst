...
.. note::
    ...
    工具链默认路径在 middleware/soc/bk7239/bk7239.defconfig 文件中定义，您也可以在项目配置文件中更改工具链路径::
...
在终端下输入下述命令编译 Armino BK7239 verify 工程::

    cd ~/armino
    make bk7239 PROJECT=verify/spe

您也可以使用 verify 工程特有的编译命令来编译 BK7239 默认工程::
...
.. note::
  ...
  Armino 框架支持通过配置 PROJECT 参数来编译任意工程，但 BK7239 尚在开发阶段，当前仅支持 verify 工程编译。
...
您可以通过工程配置文件来进行更改默认配置或者针对不同芯片进行差异化配置::
    ...
    + 工程配置文件示例：
        projects/verify/config/bk7239.config
    + 芯片配置文件示例：
...
BK7239 默认工程为 projects/app，新建工程可参考 projects/harmony 工程
...
.. figure:: ../../_static/download_uart_bk7239.png
    :align: center
    :alt: BKFIL GUI
    :figclass: align-center
